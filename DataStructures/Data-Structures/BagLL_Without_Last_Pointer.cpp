...
{
    size = 0;
    first = last = ??; // At the beginning both, first and last are null
...
bool Bag::add(string str)
{
    ...
    if ( ?? == NULL ) first = AddItem;		// first item to be added
...
bool Bag::remove(string str)		// Delete the str from the bag
{
	...
	{
		if (first == ??) // there is only one item, which will be deleted
			first = last = NULL;
		else
			first = first->??;	// first of many items is being deleted
		delete front;
		size--;
		return true;
	}
	...
	while (front != NULL)
	{
		if (front->data == ??)
		{
			...
			if (front == last) last = ??; // the last item is being deleted
			delete front;
			size--;
			return true;
		}
