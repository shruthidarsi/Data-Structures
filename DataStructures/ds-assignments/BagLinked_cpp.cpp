...
{
    link front;
    link newNode = new item(str,NULL);
    front = first;
    ...
    if(first == NULL) {
        ...
        while(front->next != NULL) front = front->next; //Iterate till last item
        front->next = newNode; //asign newNode to front->next (like last->next)
    }
    size++;
    return true;
}
